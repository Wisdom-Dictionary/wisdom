...
class DBHelper {
  ...
  void _addFolderIsDefault(Batch batch) {
    batch.execute('alter table folders add  column is_default bolean default false');
  }
...
UNION
  ...
  Future<List<WordBankFolderModel>?> getWordBankFolderList() async {
    try {
      if (database.isOpen) {
        var response = await database.rawQuery("select * from folders order by is_default desc");
  ...
  Future saveWordBankFolderList(List<WordBankFolderModel> wordBankFolderList) async {
    try {
      if (database.isOpen) {
